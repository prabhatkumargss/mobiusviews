          ...
          <nav className="hidden md:flex space-x-6">
            {navItems.map(({ label, id }) => (
              <button
                ...
                className={`font-bold border-b-2 cursor-pointer transition ${activeSection === id
          ...
          <div className="md:hidden bg-[var(--body_Color)] px-4 py-3 space-y-2">
            {navItems.map(({ label, id }) => (
              <button
                key={id}
                onClick={() => handleNavClick(id)}
                className={`block w-full text-left py-2 border-b ${activeSection === id
                    ? "text-[var(--button_Primary)] border-[var(--button_Primary)]"
